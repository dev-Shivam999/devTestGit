...
const Australiatouristvisa = () => {
  ...
  const fetchFormData = async () => {
    try {
      ...
      const response = await fetch(
        `${BackendURL}/api/forms/slug/australia-tourist-visa`
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        `${BackendURL}/api/form-submissions/slug/australia-tourist-visa`,
        {
          method: "POST",
  ...
  if (loading)
    return (
      <LoadingState message="Loading Australia Tourist Visa..." fullScreen />
    );
  if (error)
    return (
      <ErrorState
  ...
  const additionalDocsSection =
    contentSections[
      "Additional Documents for Your Australia Tourist Visa (As Applicable)"
    ] || [];
  const howToApplySection =
    contentSections["How to Apply for Australia Tourist Visa"] || [];
  const whyChooseSection =
    contentSections["Why Choose DU Global for Australia Visa"] || [];

  // Visa types data - use API data if available, otherwise static
  const visaTypesData =
    pricingPlans.length > 0 ? pricingPlans[0] : STATIC_VISA_TYPES;
    ...
    <div className="bg-white font-sans">
      ...
      <section
        ...
        <div
          ...
          style={{
            background:
              "linear-gradient(to right, rgba(0,0,0,0.7) 0%, rgba(0,0,0,0.5) 60%, rgba(179,29,29,0.6) 100%)",
          }}
        />
                      ...
                      <>
                        ...
                        {checkboxFields.map((field, index) => (
                          <label
                            ...
                            className="flex items-start gap-3 text-white text-xs cursor-pointer">
                            <input
                              type="checkbox"
                              name={field.name}
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="border-2 border-white/30 rounded-xl p-5 bg-white/10 backdrop-blur-sm">
                  ...
                  <p className="text-white font-bold text-lg">
                    {visaTypesData.processingTime ||
                      STATIC_VISA_TYPES.processingTime}
                  </p>
                </div>
            ...
            <div className="grid md:grid-cols-2 gap-6">
              {documents.map((doc, docIndex) => {
                ...
                return chunks.map((chunk, chunkIndex) => (
                  <div
                    key={`${docIndex}-${chunkIndex}`}
                    className="relative bg-white rounded-xl p-6 shadow-md border border-gray-100 overflow-hidden">
                    {/* Number */}
                    <span
                    ...
                    <div
                      ...
                      style={{
                        background:
                          "linear-gradient(135deg, transparent 50%, rgba(227,30,36,0.1) 50%)",
                      }}
                    />
                  </div>
                ));
              ...
              {additionalDocsSection.map((item, index) => (
                <div
                  ...
                  <div
                    ...
                    style={{
                      background:
                        "linear-gradient(135deg, transparent 50%, rgba(198,38,37,0.15) 50%)",
                    }}
                  />
                </div>
              ))}
          ...
          <div className="max-w-6xl mx-auto px-6">
            ...
            <div className="grid md:grid-cols-3 gap-8">
              {howToApplySection.map((item, index) => {
                const IconComponent =
                  howToApplyIcons[index % howToApplyIcons.length];
                return (
                  <div
                    key={item._id || index}
            ...
            <div className="space-y-4">
              {faqs.map((faq, index) => (
                <div
                  ...
                  {openFaq === index && (
                    <div
                      className="px-6 py-5 bg-gray-50 border-t"
                      style={{ borderColor: "#E31E24" }}>
