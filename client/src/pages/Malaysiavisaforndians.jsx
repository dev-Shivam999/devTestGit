...
const Malaysiavisaforndians = () => {
  ...
  const fetchFormData = async () => {
    try {
      ...
      const response = await fetch(
        `${BackendURL}/api/forms/slug/malaysia-visa-for-indians`
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        `${BackendURL}/api/form-submissions/slug/malaysia-visa-for-indians`,
        {
          method: "POST",
  ...
  if (loading)
    return <LoadingState message="Loading Malaysia Visa..." fullScreen />;
  if (error)
    return (
      <ErrorState
        error={error}
        ...
        <section className="py-20 bg-gray-50">
          <div className="max-w-4xl mx-auto px-6">
            ...
            <div className="mt-8 text-center">
              <button className="px-12 py-4 rounded-full font-bold text-lg transition-all duration-300 bg-[#FF1033] text-[#FFFDF5] hover:bg-[#511313] hover:text-[#FF1033] uppercase tracking-wide">
                Apply Now
              </button>
            </div>
          </div>
