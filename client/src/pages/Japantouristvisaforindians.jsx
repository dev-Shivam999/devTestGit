...
const Japantouristvisaforindians = () => {
  ...
  const fetchFormData = async () => {
    try {
      ...
      const response = await fetch(
        `${BackendURL}/api/forms/slug/japan-tourist-visa-for-indians`
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        `${BackendURL}/api/form-submissions/slug/japan-tourist-visa-for-indians`,
        {
          method: "POST",
  ...
  if (loading)
    return <LoadingState message="Loading Japan Tourist Visa..." fullScreen />;
  if (error)
    return (
      <ErrorState
        error={error}
  ...
  const introSection =
    contentSections["What Should Indians Know Before Visiting Japan?"] || [];
  const travelTipsSection =
    contentSections[
      "Essential Travel Tips for Indian Tourists Visiting Japan"
    ] || [];
  const destinationsSection =
    contentSections["Top Tourist Destinations in Japan for Indian Travelers"] ||
    [];
  const mustVisitSection =
    contentSections["Must-Visit Destinations for Indian Travelers"] || [];
  const visaFeesSection =
    contentSections[
      "Japan Tourist Visa Fees, Validity & Processing Time for Indian Citizens"
    ] || [];
  ...
  const parseVisaFees = (contentHtml) => {
    ...
    lines.forEach((line) => {
      ...
      if (line.includes("Stay period:"))
        details.stayPeriod = line.split(":")[1]?.trim();
                    ...
                    <div className="flex items-center gap-4 p-4 bg-gray-50 rounded-lg">
                      ...
                      <div>
                        ...
                        <p className="font-bold text-gray-900">
                          {details?.processingTime || "10 working days"}
                        </p>
                      </div>
                    </div>
                    <div className="flex items-center gap-4 p-4 bg-gray-50 rounded-lg">
